...
import { formatRecordJson, formatDateTime, formatDuration, formatTime } from "./api";
...
function OperationRecords({ operations, devices, initialDeviceId }: { operations: Operation[]; devices: Device[]; initialDeviceId: string }) {
  ...
  const filtered = useMemo(() => operations.filter((item) => (actor === "all" || item.actor_type === actor) && (status === "all" || item.status === status) && (deviceId === "all" || item.device_id === deviceId) && (!search || formatRecordJson(item).toLowerCase().includes(search.toLowerCase()))), [operations, actor, status, deviceId, search]);
...
function SystemRecords({ audits }: { audits: Audit[] }) {
  ...
  const filtered = useMemo(() => audits.filter((audit) => !search || formatRecordJson(audit).toLowerCase().includes(search.toLowerCase())), [audits, search]);
  return <>
    ...
    <section className="records-layout"><div className="table-pane records-table"><table><thead><tr><th>时间</th><th>操作者</th><th>动作</th><th>详情</th></tr></thead><tbody>{filtered.map((audit) => <tr key={audit.audit_id} className={selected?.audit_id === audit.audit_id ? "selected-row" : ""} onClick={() => setSelected(audit)}><td>{formatTime(audit.created_ns)}</td><td><span className={`actor ${audit.actor_type}`}>{audit.actor_type}</span><small>{audit.actor_name}</small></td><td><strong>{audit.action}</strong></td><td><code>{formatRecordJson(audit.details)}</code></td></tr>)}</tbody></table>{!filtered.length && <p className="empty-state">没有符合条件的系统事件</p>}</div><aside className="record-detail"><div className="panel-title"><span>事件详情</span></div>{selected ? <><dl><dt>Audit ID</dt><dd>{selected.audit_id}</dd><dt>操作者</dt><dd>{selected.actor_type} / {selected.actor_name}</dd><dt>时间</dt><dd>{formatTime(selected.created_ns)}</dd><dt>动作</dt><dd>{selected.action}</dd></dl><h3>详情</h3><pre>{formatRecordJson(selected.details, 2)}</pre></> : <p className="empty-state">选择一条系统事件查看详情</p>}</aside></section>
  </>;
}
